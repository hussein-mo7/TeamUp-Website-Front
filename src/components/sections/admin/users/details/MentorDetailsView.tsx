...
const projectStatusClasses: Record<
  AdminUserSupervisedProjectRecord["status"],
  string
> = {
...
const MentorDetailsView = ({ user }: MentorDetailsViewProps) => {
            ...
            rightContent={
              <div className="flex flex-wrap gap-2">
                {(user.skills.length ? user.skills : []).map((skill) => (
                  <span
                    key={skill}
                    className="rounded bg-primary-light px-2.5 py-1 font-primary text-[11px] text-primary"
                  >
                    {skill}
                  </span>
                ))}
              ...
              <div className="mt-5">
                    ...
                    <tbody>
                      {supervisedProjects.map((project, index) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 font-primary text-sm text-content">
                            {project.teamName}
                          </td>
                          <td className="px-4 py-3">
                            <span
                              className={`inline-flex rounded-full px-2.5 py-1 font-primary text-[11px] ${projectStatusClasses[project.status]}`}
                            >
                              {project.status}
                            </span>
                          </td>
                              ...
                              <div className="flex -space-x-2">
                                {project.members.map((member, memberIndex) => (
                                  <span
                                    ...
                                    className="flex h-6 w-6 items-center justify-center rounded-full border border-white bg-slate-200 text-[9px] font-semibold text-slate-600"
                                  >
                                    {member}
                                  </span>
                                ))}
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center justify-center gap-2 text-slate-400">
                              <button
                                type="button"
                                aria-label={`View ${project.teamName}`}
                              ...
                              <IconButton
                                ...
                                variant="ghost"
                                size="sm"
                                aria-label={`More actions for ${project.teamName}`}
                ...
                <div className="mt-4 flex flex-col gap-3 text-sm text-slate-500 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                      className="rounded-full px-2 py-1 text-slate-400 hover:bg-slate-100 hover:text-content"
                    >
                      ‹
                    </button>
                    <button
                    ...
                    <button
                      type="button"
                      className="rounded-full bg-primary-light px-3 py-1 font-medium text-primary"
                    >
                      2
                    </button>
                    <button
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                    <label className="relative block w-full sm:w-[180px]">
                      ...
                      <input
                        type="search"
                        placeholder="Search"
                        className="h-10 w-full rounded-full border border-slate-200 bg-white pl-9 pr-3 font-primary text-sm text-content outline-none transition-colors placeholder:text-slate-300 focus:border-primary"
                      />
                    </label>

                    <button
                      type="button"
                      className="inline-flex h-10 items-center justify-between gap-2 rounded-full border border-slate-200 bg-white px-3 text-sm font-medium text-content"
                    >
                      <span>All</span>
                      <ChevronDown className="h-4 w-4 text-slate-400" />
                    </button>
                    ...
                    <tbody>
                      {postedIdeas.map((idea, index) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <span
                              className={`inline-flex rounded-full px-2.5 py-1 font-primary text-[11px] ${ideaStatusClasses[idea.status]}`}
                            >
                              {idea.status}
                            </span>
                          </td>
                          <td className="px-4 py-3 font-primary text-sm text-slate-500">
                            {idea.date}
                          </td>
                          <td className="px-4 py-3">
                            <div className="flex items-center justify-center gap-2 text-slate-400">
